// Modified for the JPW-ABC teaching renderer: curved tuplet marks and beam geometry.
...
export class SectionsRenderer {
	...
	noteHighlightColor(note: MusicNote<NoteCharMusic>) {
		...
		const ratio = (tuneValue - lower) % 12
		let newColor = [0, 0, 0];
		for(let i = 0; i < 3; i++) {
			newColor[i] = colorSamples[lower][i] * (1 - ratio) + colorSamples[higher][i] * ratio;
		}
	...
	render(currY: number, part: SectionsRenderData, sectionCount: number, root: DomPaint, context: RenderContext, hasJumperOverlap: boolean, isFirstPart: boolean, type: 'normal' | 'compact' | 'accompany' | 'substitute') {
		...
		sections.forEach((section, sectionIndex) => {
			// 画高亮区
			if(section.idCard.lineNumber != -1 && section.idCard.masterId != '') {
				...
				section.decoration.forEach((decor) => {
					if(decor.char == 'T') {
						...
						const annoToken = new PaintTextToken(
							tripletText, new FontMetric('SparksNMN-mscore-20/400', 2.7),
							scale
						)
						const annoMeasure = annoToken.measureFast(root)
						const textGap = Math.max(annoMeasure[0] / 2 + 0.45 * scale, 0.9 * scale)
						const midLX = midX - textGap
						const midRX = midX + textGap
						if(midLX > startX + 0.5 * scale) {
							drawConnect(startX, true, midLX, false, highY + 1.55, 1.55)
						}
						if(endX > midRX + 0.5 * scale) {
							drawConnect(midRX, false, endX, true, highY + 1.55, 1.55)
						}
						annoToken.drawFast(root, midX, highY - 0.05 * scale, 'center', 'middle')
					}
				})
			}
		})
