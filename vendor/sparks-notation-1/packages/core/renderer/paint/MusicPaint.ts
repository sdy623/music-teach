// Modified for the JPW-ABC teaching renderer: vector barlines, open fonts, and print spacing.
...
export class MusicPaint {
	...
	drawSectionSeparator(context: RenderContext, x: number, y: number, sep: SectionSeparators, pos: 'before' | 'after' | 'next', fontScale: number = 1, scale: number = 1, extraStyles: ExtraStyles = {}) {
		...
		if(ch == '|' || ch == '/|') {
			this.drawJpwThinBar(x, y, scale, extraStyles)
		} else if(ch == '||' || ch == '/||') {
			this.drawJpwDoubleBar(x, y, scale, extraStyles)
		} else if(ch == '|||') {
			this.drawJpwFinalLikeBar(x, y, scale, extraStyles)
		} else if(ch == '||:' || ch == '/||:') {
			this.drawJpwRepeatBar(x, y, 'right', fontScale, scale, extraStyles)
		} else if(ch == ':||' || ch == ':/||') {
			this.drawJpwRepeatBar(x, y, 'left', fontScale, scale, extraStyles)
		} else if(ch == ':||:' || ch == ':/||:') {
			this.drawJpwRepeatBar(x, y, 'both', fontScale, scale, extraStyles)
		}
	...
	drawJpwThinBar(x: number, y: number, scale: number, extraStyles: ExtraStyles) {
		this.root.drawLine(x, y - 2.35, x, y + 2.35, 0.11, 0, scale, extraStyles)
	}

	drawJpwDoubleBar(x: number, y: number, scale: number, extraStyles: ExtraStyles) {
		this.root.drawLine(x - 0.18, y - 2.35, x - 0.18, y + 2.35, 0.11, 0, scale, extraStyles)
		this.root.drawLine(x + 0.18, y - 2.35, x + 0.18, y + 2.35, 0.11, 0, scale, extraStyles)
	}

	drawJpwFinalLikeBar(x: number, y: number, scale: number, extraStyles: ExtraStyles) {
		this.root.drawLine(x - 0.23, y - 2.35, x - 0.23, y + 2.35, 0.11, 0, scale, extraStyles)
		this.root.drawLine(x + 0.12, y - 2.35, x + 0.12, y + 2.35, 0.26, 0, scale, extraStyles)
	}

	drawJpwRepeatBar(x: number, y: number, dots: 'left' | 'right' | 'both', fontScale: number, scale: number, extraStyles: ExtraStyles) {
		if(dots == 'right') {
			this.root.drawLine(x - 0.18, y - 2.35, x - 0.18, y + 2.35, 0.26, 0, scale, extraStyles)
			this.root.drawLine(x + 0.18, y - 2.35, x + 0.18, y + 2.35, 0.11, 0, scale, extraStyles)
		} else if(dots == 'left') {
			this.root.drawLine(x - 0.18, y - 2.35, x - 0.18, y + 2.35, 0.11, 0, scale, extraStyles)
			this.root.drawLine(x + 0.18, y - 2.35, x + 0.18, y + 2.35, 0.26, 0, scale, extraStyles)
		} else {
			this.root.drawLine(x - 0.24, y - 2.35, x - 0.24, y + 2.35, 0.11, 0, scale, extraStyles)
			this.root.drawLine(x, y - 2.35, x, y + 2.35, 0.26, 0, scale, extraStyles)
			this.root.drawLine(x + 0.24, y - 2.35, x + 0.24, y + 2.35, 0.11, 0, scale, extraStyles)
		}

		const dotMetric = new FontMetric('CommonBlack/400', 0.72 * fontScale)
		const dotSides = dots == 'both' ? [-1, 1] : [dots == 'left' ? -1 : 1]
		dotSides.forEach((side) => {
			const dotX = x + side * 0.72 * fontScale * scale
			this.root.drawTextFast(dotX, y - 0.72 * fontScale, '•', dotMetric, scale, 'center', 'middle', extraStyles)
			this.root.drawTextFast(dotX, y + 0.72 * fontScale, '•', dotMetric, scale, 'center', 'middle', extraStyles)
		})
	}
	/**
	...
	drawBase(x: number, y: number, base: BaseTune, fontScale: number = 1, scale: number = 1, extraStyles: ExtraStyles = {}, dryRun: boolean = false) {
		...
		const keyGap = 0.18 * fontScale * scale
		const totalMeasure = [
			text1Measure[0] + keyGap + rootMeasure[0] + accidentalMeasure[0],
