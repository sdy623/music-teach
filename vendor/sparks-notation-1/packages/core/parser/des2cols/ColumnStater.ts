// Modified for the JPW-ABC teaching renderer: dynamic measure-density line breaking.
...
export class ColumnStater {
	...
	linifyArticle = (article: Linked2Article, context: ScoreContext, issues: LinedIssue[]): LinedArticle => {
		...
		while(sectionPtr < article.sectionCount) {
			...
			let sectionCount = this.dynamicSectionCount(article, sectionPtr, sectionCountShould)
	...
	dynamicSectionCount(article: Linked2Article, sectionPtr: number, sectionCountShould: number) {
		if(article.type == 'text') {
			return 1
		}
		const maxCount = Math.max(1, Math.min(article.sectionCount - sectionPtr, sectionCountShould))
		const budget = Math.max(30, sectionCountShould * 10.2)
		let count = 0
		let density = 0
		for(let index = sectionPtr; index < sectionPtr + maxCount; index++) {
			const nextDensity = this.sectionDensity(article, index)
			if(count > 0 && density + nextDensity > budget) {
				break
			}
			density += nextDensity
			count += 1
	...
	sectionDensity(article: Linked2Article, sectionIndex: number) {
		if(article.type == 'text') {
			return 1
		}
		let noteCount = 0
		...
		article.parts.forEach((part) => {
			...
			if(section?.type == 'section') {
				noteCount = Math.max(noteCount, section.notes.length)
				suffixCount = Math.max(suffixCount, section.notes.reduce((sum, note) => sum + note.suffix.length, 0))
				decorationCount = Math.max(decorationCount, section.decoration.length)
			}
			part.lyricLines.forEach((lyricLine) => {
				const lyricSection = lyricLine.sections[sectionIndex]
				if(lyricSection?.type == 'section') {
					lyricCount = Math.max(lyricCount, lyricSection.chars.length)
				}
			})
		})
		const quarters = Frac.toFloat(article.sectionFields[sectionIndex]?.[1] ?? Frac.create(4))
		const notePressure = Math.max(1, noteCount) * 0.72
		const lyricPressure = lyricCount * 0.34
		const ornamentPressure = suffixCount * 0.18 + decorationCount * 0.35
		const timePressure = Math.max(1, quarters) * 0.45
		return notePressure + lyricPressure + ornamentPressure + timePressure
	}
